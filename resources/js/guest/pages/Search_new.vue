    ...
    <div class="container">
      ...
      <div class="row">
        ...
        <div class="card-group col-12 px-sm-5 px-md-5">
          ...
          <div v-else>
            {{this.pageLinks}} 
            <h5> Risultati trovati: ({{found_results}})</h5>
          </div>
          <div
...
export default {
  ...
  methods: {
    getApi() {
      axios
        ...
        .then((response) => {
          ...
          this.found_results = response.data.data.foundResults;
          this.pages = $this.found_results / this.items_per_page;

          if (this.found_results > this.items_per_page){
    ...
    getPageLinks() {
       axios
        .get(`/api/doctors/get/page_links`, {
          params: {
            items: this.sponsor.length + this.not_sponsor.length,
            route: this.$route,
          },
        })
        .then((response) => {
          console.log(response.data);
          this.page_links = response.data.data;
        });
    },
    checkIfEmpty() {
      if (!this.title) {
